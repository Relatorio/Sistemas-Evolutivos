...
// NOVO: Inicializa o ponteiro de arquivo como NULL
...
Individual run_ga_cycle(double (*fitness_func)(Individual, const void*), const void* extra_param, int is_shape_opt) {
    ...
    // ESCREVE O CABEÇALHO APENAS SE O ARQUIVO ESTIVER ABERTO
    if (ga_csv_file != NULL) {
    ...
    for (int gen = 0; gen < MAX_GENERATIONS; gen++) {
                ...
                if (stagnation_counter >= STAGNATION_LIMIT) {
                    if (mutation_rate < MUTATION_MAX) { mutation_rate *= 1.2; }
                        ...
                        if (repulsion_mode_counter >= RESET_AFTER_REPULSION_GENS) {
                            ...
                            // Avisa no terminal que houve reset (opcional)
                            // printf(" [GA LOG] Reset de Populacao na Geracao %d\n", gen+1);
        ...
        if (ga_csv_file != NULL) {
            ...
            // Usamos gen % 5 == 0 ou gen == 0 para não salvar TODAS as linhas se for muita coisa
            // Mas para o gráfico detalhado, salvar todas é melhor.
        ...
        // Barra de progresso visual no terminal (opcional)
    ...
    printf("\n"); // Quebra linha da barra de progresso
